...
inotifywait -m -e modify --format '%w%f' --include "(.*.html|.*.ts)$" index_template.html model generators web tests/web tests/parser tests/generators tests/model web/visualizations |
    while read path; do
