function ast(yy) {
    ...
    yy.result(indent(JSON.stringify({visualizer:r.getVisualizer()})));
    yy.result(indent(JSON.stringify({direction:r.getDirection()})));
    yy.result(indent(JSON.stringify({start:r.getStart()})));
    yy.result(indent(JSON.stringify({equal:r.getEqual()})));
